...
describe('Assertions', () => {
  ...
  it('each() - interacting with multiple web elements', () => {
    ...
    const options = ['Apple', 'Microsoft', 'Tesla']

    cy.get('.checkbox').each(($el, index) => {
      cy.wrap($el)
...
*/
   ...
   */
    ...
    const opt = ['Microsoft', 'Apple']

    cy.get('[id^=company_dropdown]').each(($el, index) => {
      cy.wrap($el)
        .should('be.enabled')
        .select(opt[index])
        .and('have.value', opt[index])
    })
